...
CPlayerAttack5State::CPlayerAttack5State( CCharacter * _pCharacter, const std::string &_Name )
	...
	, m_fAttackYaw(0.f)
{
	m_pCallback				= static_cast<CGameProcess*>(CORE->GetProcess())->GetAnimationCallbackManager()->GetCallback(_pCharacter->GetName(),"attack5");
...
bool CPlayerAttack5State::OnMessage( CCharacter* _pCharacter, const STelegram& _Message )
{
	...
	else if( _Message.Msg == Msg_Push )
	{
		...
		LOGGER->AddNewLog(ELL_INFORMATION, "CPlayerAttack5State::OnMessage -> PUSHED!!");
		return true;
	}
