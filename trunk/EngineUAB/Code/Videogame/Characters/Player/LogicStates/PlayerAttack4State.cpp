...
CPlayerAttack4State::CPlayerAttack4State( CCharacter * _pCharacter, const std::string &_Name )
	...
	, m_fAttackYaw(0.f)
{
	...
	m_pCallback			= m_pProcess->GetAnimationCallbackManager()->GetCallback(_pCharacter->GetName(),"attack4");
...
void CPlayerAttack4State::OnEnter( CCharacter* _pCharacter )
{
#if defined _DEBUG
	if( CORE->IsDebugMode() )
	{
		CORE->GetDebugGUIManager()->GetDebugRender()->SetStateName("Attack 4");
	}
#endif
...
void CPlayerAttack4State::Execute( CCharacter* _pCharacter, float _fElapsedTime )
{
	if( m_bFirstUpdate )
	{
...
void CPlayerAttack4State::OnExit( CCharacter* _pCharacter )
{
...
bool CPlayerAttack4State::OnMessage( CCharacter* _pCharacter, const STelegram& _Message )
{
	...
	else if( _Message.Msg == Msg_Push )
	{
		...
		LOGGER->AddNewLog(ELL_INFORMATION, "CPlayerAttack4State::OnMessage -> PUSHED!!");
		return true;
	}
...
bool CPlayerAttack4State::CalculateAngleMovement( CCharacter *_pCharacter, float &_fAngle )
{
	bool l_bMove = false;			
