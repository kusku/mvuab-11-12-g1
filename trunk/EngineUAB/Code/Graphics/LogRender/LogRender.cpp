...
void CLogRender::Render (CRenderManager* renderManager, CFontManager* fm, CColor color)
{
	if( m_bIsVisible )
	{
		...
		for (; it != itEnd; ++it)
		{
			...
			uint32 win_w, win_h;
			Vect2i screen = renderManager->GetScreenSize();
			win_w = screen.x;
			win_h = screen.y;
	...
	else
	{
		...
		action2Input->GetActionInfo(ACTION_LOGGER, info);
		std::string l_sInfo;
		baseUtils::FormatSrting(l_sInfo, "Press %s to view the Logger", info.c_str());
