...
//---Constructor
CGUIEditableTextBox::CGUIEditableTextBox(	uint32 windowsHeight, uint32 windowsWidth, float height_precent, float witdh_percent,
											const Vect2f position_percent, CColor textColor, uint32 fontID, std::string lit, 
											uint32 textHeightOffset, uint32 textWidthOffset, bool isVisible,  bool isActive)
...
CGUIEditableTextBox::~CGUIEditableTextBox()
{
	CHECKED_DELETE(m_Result);
}

//---------------Interfaz de GuiElement----------------------
void CGUIEditableTextBox::Render (CRenderManager *renderManager, CFontManager* fm)
{
...
void CGUIEditableTextBox::Update(CInputManager* intputManager, float elapsedTime)
{
...
void CGUIEditableTextBox::Remove1character()
{
	if (m_uCursorPos > 0)
	{
...
bool CGUIEditableTextBox::IsReturnPress ()
{
	if( m_bReturnPress )
	{
		m_bReturnPress = false;
...
void CGUIEditableTextBox::ProcessNewEntries (CInputManager* inputManager)
{
	uint32 index= 237;
...
void CGUIEditableTextBox::SetFont (CColor textColor, uint32 fontID)
{
	m_uFontID		= fontID;
