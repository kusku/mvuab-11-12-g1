class 'CRabbitPursuitState' (CState)
	function CRabbitPursuitState:__init(name) 
		...
		-- print_logger(0, "CRabbitPursuitState:__init->Inicio del estado PURSUIT de la caperucita")
	end
	...
	function CRabbitPursuitState:Execute(_CCharacter, _elapsed_time)
	...
	-- function :Execute(_CCharacter, _elapsed_time):MoveOnSide(_CCharacter)
		-- _CCharacter.behaviors:pursuit_off()
		-- print_logger(1, ":Execute(_CCharacter, _elapsed_time):MoveOnSide(_CCharacter)")
		...
		-- if ( self.action_started == false ) then 
			...
			-- self.new_direction = self.new_direction + _CCharacter.properties.min_speed * _elapsed_time
		...
		-- else 
			...
			-- -- -- -- _CCharacter:face_to( _CCharacter.behaviors.seek.target , _elapsed_time )
			-- -- -- -- _CCharacter:move_to2( _CCharacter.behaviors.seek.target , _elapsed_time )
			-- -- else
				-- -- -- _CCharacter.behaviors:seek_off()
				-- -- -- _CCharacter.steering_entity.velocity = Vect3f(0,0,0)
