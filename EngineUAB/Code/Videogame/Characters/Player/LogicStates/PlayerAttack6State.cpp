...
CPlayerAttack6State::CPlayerAttack6State( CCharacter * _pCharacter, const std::string &_Name )
	...
	, m_fAccelerationMovement( _pCharacter->GetProperties()->GetAccelerationAttack6() )
...
void CPlayerAttack6State::OnEnter( CCharacter* _pCharacter )
{
	...
	if( CORE->IsDebugMode() )
	{
		CORE->GetDebugGUIManager()->GetDebugRender()->SetStateName("Attack 6");
	}
#endif
	...
	CORE->GetSoundManager()->PlayEvent( _pCharacter->GetSpeakerName(), "Play_EFX_Caperucita_attack3");
}

void CPlayerAttack6State::Execute( CCharacter* _pCharacter, float _fElapsedTime )
{
	if( m_bFirstUpdate )
	{
...
void CPlayerAttack6State::SetParticlePosition( CCharacter* _pCharacter )
{
